...
def plot(sids, paper=False):
    ...
    for sid in sids:
        ...
        try:
            all_dict[cu_count][cu_cores] = all_dict[cu_count][cu_cores].append(pd.Series(((tufs['aec_after_exec'].max() - tufs['asic_get_u_pend'].min() - cu_runtime) / (cu_cores * cu_count))))
        except:
    ...
    loc = 'upper right'
    mp.pyplot.legend(labels, loc=loc, fontsize=LEGEND_FONTSIZE)
    if not paper:
    ...
    mp.pyplot.ylabel("$TTC_{overhead}$ per core (s)", fontsize=LABEL_FONTSIZE)
    mp.pyplot.ylim(0)
